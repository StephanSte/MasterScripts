    ...
    public class CameraHandler : MonoBehaviour
    {
        ...
        public void HandleCameraRotation(float delta, float mouseXInput, float mouseYInput)
        {
            if (inputHandler.lockOnFlag == false && currentLockOnTarget == null)
            {
                ...
                if (float.IsNaN(lookAngle) || lookAngle > 360 || lookAngle < -360)
                {
                    lookAngle = 45;
                }
                pivotAngle -= (mouseYInput * pivotSpeed) / delta;
                pivotAngle = Mathf.Clamp(pivotAngle, minimumPivot, maximumPivot);
                if (float.IsNaN(pivotAngle))
                {
                    pivotAngle = 0;
                    return;
                }
            ...
            else
            {
                ...
                eulerAngle.y = 0;
                cameraPivotTransform.localEulerAngles = eulerAngle / 2;
            }
        }
        ...
        public void SetCameraHeight()
        {
            ...
            Quaternion newLockedRotation = Quaternion.Euler(-6.3f, 0,0);
            Vector3 newUnlockedPosition = new Vector3(0, unlockedPivotPosition);

            if (currentLockOnTarget != null)
            {
                cameraPivotTransform.transform.localPosition = Vector3.SmoothDamp(cameraPivotTransform.transform.localPosition, newLockedPosition, ref velocity, Time.deltaTime);
                cameraTransform.transform.localRotation = newLockedRotation;
            }
            else
            {
